...
class LocalDB extends DatabaseUtility {
  ...
  @override
  void deleteDB(String name) async => databaseFactory
      .deleteDatabase(join(await getDatabasesPath(), '$name.db'));
}
...
class FirebaseDB extends DatabaseUtility {
  ...
  Future<List<User>> getUsers() async {
    ...
    final users = result.docs
        .map(
            (e) => User(phoneNumber: e['phoneNumber'], username: e['username']))
        .toList();
  ...
  void saveMessage(Message data) async {
    ...
    if (await db
      ...
      db.collection("messages").add(message).then((DocumentReference doc) =>
